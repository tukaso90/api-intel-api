...
const totalFindings = endpoints.length * 2;  // ~2 findings/endpoint avg
  ...
  const avgScore = totalEndpoints ? Math.round(
    perEndpoint.reduce((sum, ep) => sum + (ep as any).endpointScore, 0) / totalEndpoints
  ) : 0;
  ...
  return {
    ...
    totalFindings,
    avgScore,
    perEndpoint: perEndpoint as any[]
  } as any;
}
